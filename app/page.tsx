...
// --- SUB-COMPONENT: MERMAID DIAGRAM (DYNAMIC DESCRIPTION FIXED) ---
const Mermaid = ({ 
  chart, 
  name, 
  description 
  ...
  return (
    <div className="w-full space-y-6">
      <div 
        key={chart} 
        className="mermaid flex justify-center items-center w-full min-h-[300px]" 
        ref={ref}
      >
        {chart}
      </div>
      ...
      <div className="flex gap-4 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md animate-in fade-in slide-in-from-bottom-2 duration-500">
        <div className="p-2 bg-indigo-500/20 rounded-xl self-start">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-400 leading-relaxed">
            {description || "This diagram visualizes the relationships and architectural flow of the pattern components."}
          </p>
        </div>
      </div>
    </div>
...
export default function DesignPatternsPage() {
  ...
  // Safely extend PatternData type if diagramDescription isn't in your base interface
  const pattern = (PATTERNS_REGISTRY[currentId] as PatternData & { diagramDescription?: string });
  ...
  useEffect(() => {
    setExpandedCat(pattern.category);
  }, [currentId, pattern.category]);
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-950 text-white font-sans overflow-hidden relative">
          ...
          {Object.entries(categoriesMap).map(([categoryName, patterns]) => {
            ...
            return (
              <div key={categoryName} className="space-y-2">
                <button
                  onClick={() => setExpandedCat(isExpanded ? null : categoryName)}
        ...
        <div className="max-w-6xl mx-auto p-12 md:p-20 space-y-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            <div className="lg:col-span-7 space-y-12">
              ...
              <div className="group relative bg-gradient-to-br from-slate-900/50 to-indigo-900/30 border border-white/10 rounded-3xl overflow-hidden shadow-2xl backdrop-blur-sm">
                <button 
                  ...
                  className="absolute top-6 right-6 p-3 bg-white/10 border border-white/20 rounded-xl shadow-lg opacity-0 group-hover:opacity-100 transition-all z-10 text-white hover:bg-white/20"
                >
                  <Maximize2 size={18} />
                </button>
                <div className="p-8">
                  <Mermaid 
                    ...
                    name={pattern.name} 
                    description={pattern.diagramDescription} 
                  />
                </div>
              </div>
              ...
              <div className="space-y-6">
                <h3 className="text-sm font-bold uppercase text-indigo-300 tracking-[0.2em] flex items-center gap-2">
                  <Sparkles size={16} /> Architecture Components
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {pattern.roles.map((role: PatternRole, i: number) => (
                    <div key={i} className="p-6 border border-white/10 rounded-2xl bg-slate-900/40 hover:bg-indigo-900/20 transition-all backdrop-blur-sm">
                      <div className="flex items-center gap-3 mb-3 text-white font-bold text-sm uppercase">
            ...
            <div className="lg:col-span-5">
              <div className="border border-white/10 rounded-3xl overflow-hidden shadow-2xl sticky top-8 flex flex-col bg-slate-900/80 backdrop-blur-xl">
                <div className="bg-white/5 px-6 py-4 border-b border-white/10 flex justify-between items-center">
                  <div className="flex gap-4">
                    {LANGUAGES.map(l => (
                      <button 
                        key={l} 
                        onClick={() => setLang(l)} 
                        className={`text-xs font-bold uppercase transition-all pb-1 ${
                          lang === l ? "text-white border-b-2 border-indigo-400" : "text-slate-400 hover:text-slate-200"
                        }`}
                      >
                        {l}
                      </button>
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => setModalType("code")} className="p-2 hover:bg-white/10 rounded-lg transition-all text-slate-400 hover:text-white">
                      <Maximize2 size={16} />
                    </button>
                    <button onClick={handleCopy} className="p-2 hover:bg-white/10 rounded-lg transition-all text-slate-400 hover:text-white">
      ...
      <FullscreenModal isOpen={modalType === "diagram"} onClose={() => setModalType(null)} title={`UML Diagram: ${pattern.name}`}>
        <div className="flex-1 overflow-auto bg-slate-900/50 rounded-3xl border border-white/10 flex items-center justify-center p-12">
          <div className="min-w-[800px]">
            <Mermaid chart={pattern.diagram} name={pattern.name} description={pattern.diagramDescription} />
          </div>
        </div>
      </FullscreenModal>

      <FullscreenModal isOpen={modalType === "code"} onClose={() => setModalType(null)} title={`Source Code: ${pattern.name} (${lang})`}>
        <div className="flex-1 overflow-y-auto rounded-3xl border border-white/10 bg-slate-900/80 custom-scrollbar">
          <SyntaxHighlighter language={lang} style={tomorrow} customStyle={{ padding: '3rem', fontSize: '16px', background: 'transparent' }}>
            {pattern.code[lang]}
          </SyntaxHighlighter>
        </div>
      </FullscreenModal>
